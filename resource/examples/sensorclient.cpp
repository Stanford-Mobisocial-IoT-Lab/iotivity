                    ...
                    const int& eCode, const int& sequenceNumber)
{
    ...
    try
    {
        if(eCode == OC_STACK_OK && sequenceNumber != OC_OBSERVE_NO_OPTION)
        {
            ...
            rep.getValue("temperature", mylight.m_temperature);
            rep.getValue("humidity", mylight.m_humidity);

            //std::cout << "\tstate: " << mylight.m_state << std::endl;
            //std::cout << "\tpower: " << mylight.m_power << std::endl;
            //std::cout << "\tname: " << mylight.m_name << std::endl;
            std::cout << "\tNum        : " << ++num << std::endl;
            std::cout << "\tHumidity   : " << mylight.m_humidity << "%" << std::endl;
            std::cout << "\tTemperature: " << mylight.m_temperature << std::endl;
	    //std::cout << "\tthe count is: " << observe_count() << std::endl;

            //if(observe_count() >= 11)
            if(mylight.m_humidity > 70)
            {
		std::cout << "================================================================" << std::endl;
		std::cout << "=================The humidity is now above 70 now!==============" << std::endl;
		std::cout << "=====================Turning on the light.........==============" << std::endl;
		std::cout << "================================================================" << std::endl;
		putLightRepresentation2(curResource);
...
void onPut2(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
    std::cout << "sucessfully send the command!" << std::endl;
}
...
void getLightRepresentation(std::shared_ptr<OCResource> resource)
{
    if(resource)
    {
        ...
        QueryParamsMap test;  		// std::map<std::string, std::string>
    ...
    {
        {
            ...
            if(discoveredResources.find(resource->uniqueIdentifier()) == discoveredResources.end()) // If first time discover
            {
...
void printUsage()
{
    std::cout << std::endl;
    std::cout << "---------------------------------------------------------------------\n";
    std::cout << "Usage : sensorclient <ObserveType>" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    PlatformConfig cfg {
        OC::ServiceType::InProc, // OutOfProc
        OC::ModeType::Both,      // Server, Client, Both, Gateway
        "0.0.0.0",               // ipAddress
        0,                       // portNumber
    ...
    try
    {
        ...
        // Find all resources
        requestURI << OC_RSRVD_WELL_KNOWN_URI;// << "?rt=core.light";  // "/oic/res"

        OCPlatform::findResource("", requestURI.str(),
                CT_DEFAULT, &foundResource);   // CT_DEFAULT = 0
