...
public:
    /// Constructor
    LightResource()
        :m_name("Xiangyu's device"), m_state(false), m_power(0), m_lightUri("/a/light"),
                m_resourceHandle(nullptr), m_humidity(0.0), m_temperature(0.0) {
        ...
        m_lightRep.setValue("humidity", m_humidity);
        m_lightRep.setValue("temperature", m_temperature);
    }
    ...
    void createResource()
    {
        ...
        OCStackResult result = OCPlatform::registerResource(
                                    m_resourceHandle/*magic*/, resourceURI/*string*/, resourceTypeName/**/,
    ...
    void put(OCRepresentation& rep)
    {
        try {
            ...
            {
		if (m_power == 66) {
		  cout << "The action is triggered! Turning on the light..." << endl;
		  system("python ~/my_programs/led.py &");
 		}
...
OCEntityHandlerResult entityHandler(std::shared_ptr<OCResourceRequest> request)
{
    ...
    if(request)
    {
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            auto pResponse = std::make_shared<OC::OCResourceResponse>(); // This is what will be responded back
        ...
        if(requestFlag & RequestHandlerFlag::ObserverFlag)
        {
            ...
            char flag = '0';
	    cout << "Waiting for approval of the device: " << endl;
            while (flag == '0') {
              ifstream myfile("switch");
	      ...
	      sleep(1);
            ...
            if(ObserveAction::ObserveRegister == observationInfo.action && flag == '1')
            {
...
void * ChangeLightRepresentation (void *param)
{
    ...
    {
        sleep (3);
	// Check a certain state here and if it is valid, if valid, do 
	// all the work inside the { } below. 
	static double pre_humidity = 0.0, pre_temperature = 0.0;
	double cur_humidity = 0.0, cur_temperature = 0.0;
	bool changed = false;
	...
	//cout << "******************************************************" << endl;
        ...
        if (myfile.is_open())
        {
	  cout << "========================checking the data from the sensor============================" << endl;
	  string line;
	  getline(myfile, line);
	  cur_humidity = stod(line);
	  getline(myfile, line);
	  cur_temperature = stod(line);

	  cout << "The humidity is   : " << cur_humidity << "%" << endl;
	  cout << "The temperature is: " << cur_temperature << endl;
	  cout << "=====================================================================================" << endl;
          myfile.close();
        }

	changed = (cur_humidity != pre_humidity) || (cur_temperature != pre_temperature);
        pre_humidity = cur_humidity;
	pre_temperature = cur_temperature;
        ...
        if (changed)
        {
            ...
            //lightPtr->m_power += 10;
	    lightPtr->m_humidity = cur_humidity;
	    lightPtr->m_temperature = cur_temperature;

            //cout << "\nPower updated to : " << lightPtr->m_power << endl;
            //cout << "\nHumidity updated to : " << lightPtr->m_humidity << endl;
            //cout << "\nTemperature updated to : " << lightPtr->m_temperature << endl;
            cout << "The humidity or the temperature has changed. " << endl;
...
{
    std::cout << std::endl;
    std::cout << "Usage : sensorserver <value>\n";
